...
def readClusteringResults(nomFi):
	...
	for l in range(len(lines)-1):
		Seq_nom=lines[l].split("\t")[1].split("|")[0]
		Seq_nom[-1].rstrip()
	...
	Seq_nom=lines[-1].split("\t")[1].split(" ")
	cluster=lines[-1].split("\t")[0]
	if len(Seq_nom) != 0:
		Clustering_lables[cluster.rstrip()]= Seq_nom
...
def Plot(Clustering_lables,FastaFile):
	...
	j = 1
	for i in clone_list :
		line_to_write = "Clone number" + str(j) + "\t"+ str(i/float(seq_number)) + "\n"
	...
	plt.xlim(0,100)
	plt.xlabel('Clone')
	plt.ylabel('relative Clone size')
	fig.savefig(FastaFile.split(".")[0]+'.png', dpi=500)
	plt.show()
#####################################################################
def main():
	usage = "usage: %prog [options] -f FastaFile -c ClusteringFile"
